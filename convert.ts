...
/**
 * SQLのカラム値を解析し、HEX(Blob)形式の場合はUTF-8テキストとしてデコードする
 * 通常のテキストの場合はエスケープを解除して返す
 */
function parseSqlTextColumn(val: string): string {
    if (!val || val === 'NULL' || val === undefined) return '';

    // HEX形式 (X'...') の場合
    if (val.startsWith("X'") || val.startsWith("x'")) {
        // 末尾の ' を除去して中身を取り出す
        const hex = val.slice(2, -1);
        try {
            return Buffer.from(hex, 'hex').toString('utf8');
        } catch (e) {
            console.warn('HEX decoding failed, returning raw value');
            return val;
        }
    }
...
async function main() {
    ...
    for (const rawRow of iterateSqlValues(sql)) {
        ...
        const text = parseSqlTextColumn(rawCols[14]);
        const text_more = parseSqlTextColumn(rawCols[15]);
